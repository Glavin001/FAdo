...
class IPTProp(CodeProperty):
    ...
    def notMaxStatW(self, aut, ell, n=2000):
        ...
        while tr <= n:
            w = pickFrom(s, ell)
            if b.evalWordP(w) == False:  return w
            tr += 1
        return None    # in this case, tr == n+1

    def makeCode(self, N, ell, s, n=2000):
        ...
        :rtype: tuple"""
        W = [pickFrom(s, ell)]
        aut = fl.FL(W).trieFA().toNFA()
        cnt = 1
        while cnt < N:
            w = self.notMaxStatW(aut, ell, n)
            if w is None: break
            W.append(w)
            aut = fl.FL(W).trieFA().toNFA()
            cnt += 1
